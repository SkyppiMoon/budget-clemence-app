...
async function reloadCurrentMonth() {
  ...
  data.categories.filter(category => {
    ...
    return category.isEnabled || actual !== 0;
      ...
      cats.forEach(c=>{
        ...
        const displayedBudget = c.isEnabled ? c.budget : 0;
        const pct = displayedBudget > 0
          ? Math.min(100, (actual / displayedBudget) * 100)
          : (actual > 0 ? 100 : 0);
        const over = actual > displayedBudget && displayedBudget > 0;
        const excludedLabel = c.isEnabled ? '' : ' · Exclue ce mois';

        html += `<div class="pbar-row">
          <div class="pbar-top">
            <span class="pbar-name">${escapeHtml(c.name)}${excludedLabel}</span>
            <span class="pbar-figures">${eur(actual)} / ${eur(displayedBudget)}</span>
          </div>
          <div class="pbar-track">
            <div class="pbar-fill ${over?'over':cls}" style="width:${pct}%"></div>
          </div>
        </div>`;
      });
      html += `</div>`;
  ...
  function renderBudget(){
    ...
    ['oblig','besoins','envies'].forEach(type=>{
      ...
      el.innerHTML = cats.map(c=>{
        ...
        return `<div class="cat-item ${c.isEnabled ? '' : 'cat-item-disabled'}">
          <div class="cat-left">
            <div>
              <div class="cat-name">${escapeHtml(c.name)}</div>
              <div class="cat-budget">
                ${c.isEnabled ? `Budget ${eur(c.budget)}` : 'Exclue de ce mois'}
                · réel ${eur(actual)}${dueTxt}
              </div>
            </div>
          </div>
          <div class="cat-actions">
            <label class="month-toggle" title="Inclure ou exclure cette catégorie pour le mois">
              <input type="checkbox" data-toggle-cat="${c.id}" ${c.isEnabled ? 'checked' : ''}>
              <span class="month-toggle-slider"></span>
            </label>
            <button class="icon-btn" data-edit-cat="${c.id}" title="Modifier le budget du mois">✎</button>
            <button class="icon-btn" data-del-cat="${c.id}" title="Supprimer définitivement la catégorie">✕</button>
          </div>
        </div>`;
    ...
    document.querySelectorAll('[data-toggle-cat]').forEach(toggle => {
      toggle.addEventListener('change', async () => {
        const category = data.categories.find(
          item => item.id === toggle.dataset.toggleCat
        );

        if (!category || !category.monthlyBudgetId) {
          toggle.checked = category?.isEnabled ?? false;
          return;
        }
        ...
        try {
          const updated = await updateMonthlyCategoryBudget(
            category.monthlyBudgetId,
            { isEnabled: toggle.checked }
          );

          category.isEnabled = Boolean(updated.is_enabled);
          render();
        } catch (error) {
          console.error(
            'Erreur pendant la modification du budget mensuel :',
            error
          );
          toggle.checked = category.isEnabled;
          alert('Impossible de modifier cette catégorie pour le mois.');
        } finally {
    ...
    document.querySelectorAll('[data-del-cat]').forEach(button => {
      button.addEventListener('click', async () => {
        ...
        if (!confirm(
          'Supprimer définitivement cette catégorie ? Ses budgets de tous les mois seront supprimés.'
        )) {
          return;
        }
  ...
  function openCategorySheet(editId){
    const existing = editId
      ? data.categories.find(c => c.id === editId)
      : null;
    ...
    openSheet(`
      ...
      <div class="field"><label>Budget pour ${monthLabel(data.currentMonth)} (€)</label><input type="number" step="0.01" min="0" id="fBudget" value="${existing ? existing.budget : ''}"></div>
      <div class="field"><label class="checkbox-line"><input type="checkbox" id="fEnabled" ${!existing || existing.isEnabled ? 'checked' : ''}> Inclure cette catégorie dans ce mois</label></div>
    ...
    document.getElementById('saveCat').addEventListener('click', async () => {
      ...
      const dueDay = parseInt(document.getElementById('fDue')?.value, 10) || null;
      ...
      try {
        if (existing) {
          const updatedCategory = await updateCategory(existing.id, {
            name,
            type: selectedType,
            budget: existing.defaultBudget,
            isBill: selectedType === 'oblig',
            dueDay: selectedType === 'oblig' ? dueDay : null
          });

          const updatedMonthlyBudget = await updateMonthlyCategoryBudget(
            existing.monthlyBudgetId,
            { budget, isEnabled }
          );

          Object.assign(
            existing,
            mapCategory(updatedCategory, updatedMonthlyBudget)
          );
        } else {
          ...
          const createdMonthlyBudget = await createMonthlyCategoryBudget(
            createdCategory.id,
            data.currentMonth,
            budget,
            isEnabled
          ...
          data.categories.push(
            mapCategory(createdCategory, createdMonthlyBudget)
          );
        }
        ...
        console.error(
          'Erreur pendant l’enregistrement de la catégorie :',
          error
        );
